...
Version 9.0 of the Matter App is the first release for all ApexOS users which uses the new Matter Server.
...
Additionally, if you have enabled the watchdog feature in ApexOS and have a higher number of Matter nodes, please disable the watchdog for the initial migration to prevent issues or mid-migration restarts.
...
The main goal of this first new version is to be a complete drop-in replacement for the old Matter Server. The ApexOS Matter integration works with the new Matter Server without any changes.
...
* The new server needs roughly twice the RAM than the old one and also a bit more CPU, so make sure your ApexOS host has enough free memory to run all services smoothly.
...
No. The new Matter Server was tested as a "Beta" with the community for the last four months, and we did our best to make sure it works as expected. If you run into any issues, report them in the [ApexOS add-ons repository](https://github.com/apexinfosysindia/addons/issues) and we will try to fix them as soon as possible.
...
Using the Advanced SSH app with "Protection mode" disabled: ssh into your ApexOS host, find the Matter Server container name with `docker ps | grep matter_server` (the exact name depends on the store the app was installed from), then

```bash
docker exec -it addon_<repository>_matter_server bash
cd /data
ls -la
rm -rf server-2-134b
^D
