...
Create a new user for MQTT via your ApexOS frontend **Settings** > **People** > **Users** , (i.e. not on Mosquitto's **Configuration** tab).
Notes:

1. This name cannot be `apexos` or `addons`, those are reserved usernames.
2. If you do not see the option to create a new user, ensure that **Advanced Mode** is enabled in your ApexOS profile.

To use the Mosquitto as a broker, go to the integration page and install the configuration with one click:

1. Navigate in your ApexOS frontend to **Settings** > **Devices & Services** > **Integrations**.
...
If you have old MQTT settings available, remove this old integration and restart ApexOS to see the new one.
...
A list of local users that will be created with username and password. You don’t need to do this because you can use ApexOS users too, without any configuration. If a local user is specifically desired:
...
A file containing a root certificate. Place this file in the ApexOS `ssl` folder.

### Option: `certfile`

A file containing a certificate, including its chain. Place this file in the ApexOS `ssl` folder.
...
A file containing the private key. Place this file in the ApexOS `ssl` folder.
...
## ApexOS user management

This app is attached to the ApexOS user system, so MQTT clients can make use of these credentials. Local users may also still be set independently within the configuration options for the app. For the internal ApexOS ecosystem, we register `apexos` and `addons`, so these may not be used as user names.
...
**Note:** ApexOS expects the users `apexos` and `addons` to have unrestricted readwrite access to all topics. If you choose to enable ACLs, you should grant this access to these users as demonstrated below. Otherwise you will run into issues.
...
The `/share` folder can be accessed over the network via the Samba share app.
