...
If you're using Apex Connect+ you may find the correct value for this on the
...
3. Toggle the "Show in sidebar" to add it to your Apex Connect+ side bar.
4. Start the add-on.

After installing and starting this add-on, access the deCONZ WebUI ("Phoscon")
with "WEB UI" button.

## Configuring the Apex Connect+ deCONZ integration

By default, Apex Connect+ has the `discovery` integration enabled, which
...
In case you don't have `discovery` enabled on your Apex Connect+ instance,
follow these instructions to configure the deCONZ integration:

<https://www.apexinfosys.in/integrations/deconz/>

## Migrating to this Add-on

To migrate deCONZ to Apex Connect+ and this add-on, backup your deCONZ config via the
...
Adjust port and possibly hostname if you've changed it in Apex Connect+ host system
settings.
...
### My gateway shows up in Apex Connect+ with ID 0000000000000000
...
This might cause issues in Apex Connect+, like having no devices.
It also might cause an issue when the add-on has internal changes and next
fails to communicate new settings to Apex Connect+.
...
3. In Apex Connect+, remove the current integration you have for deCONZ.
4. Restart Apex Connect+.
...
7. Restart the add-on and next, restart Apex Connect+ once more.
...
to inform Apex Connect+ of changed internal settings, which happened in 4.x.
...
1. In Apex Connect+, remove the current integration you have for deCONZ.
2. Restart Apex Connect+.
