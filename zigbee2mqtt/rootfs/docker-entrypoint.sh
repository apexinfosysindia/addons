...
# Workaround for a supervisor startup race that leaves add-on options
# unavailable for a short period after boot.
...
# NOTE: the *_HOMEASSISTANT_* variable names below are Zigbee2MQTT's own
# configuration schema (its `homeassistant:` settings section) and must not be
# renamed. The values, however, are ApexOS-specific: ApexOS core uses the MQTT
# discovery prefix "apexos" (not the Zigbee2MQTT built-in default), so the
# discovery and status topics are pinned to match it here.
export ZIGBEE2MQTT_CONFIG_HOMEASSISTANT_ENABLED='true'
export ZIGBEE2MQTT_CONFIG_HOMEASSISTANT_DISCOVERY_TOPIC='apexos'
export ZIGBEE2MQTT_CONFIG_HOMEASSISTANT_STATUS_TOPIC='apexos/status'
