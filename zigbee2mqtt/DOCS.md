...
2. Find the "Zigbee2MQTT" app and click it.
3. Click on the "INSTALL" button.

This app requires an MQTT broker (for example the Mosquitto broker app). If the
broker app is installed, the connection is detected and configured
automatically.

## Pairing

By default the app has `permit_join` set to `false`. To allow devices to join you need to activate this after the app has started. You can now use the [built-in frontend](https://www.zigbee2mqtt.io/information/frontend.html) to achieve this. For details on how to enable the built-in frontend see the next section.
...
The app pins Zigbee2MQTT's discovery topic to `apexos` (and its status topic to
`apexos/status`), which is the default MQTT discovery prefix used by ApexOS.
These values are set by the app at startup and cannot be overridden from
Zigbee2MQTT's own `configuration.yaml`. If you changed the discovery prefix of
the MQTT integration in ApexOS, contact support.
...
- Depending on your configuration, the MQTT server config may need to include the port, typically `1883` or `8883` for SSL communications. For example, `mqtt://core-mosquitto:1883` for the ApexOS Mosquitto app.
- To find out which serial ports you have exposed go to **Settings → System → Hardware**
